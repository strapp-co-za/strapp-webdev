...
serve(async (req: Request) => {
	...
	try {
		...
		return new Response(JSON.stringify({ error: "Failed to send email", details: String(err), field: 'form' }), {
			status: 502,
			headers: corsHeaders(origin),
		});
	}
	...
	if (!resendRes.ok) {
		return new Response(JSON.stringify({ error: resendData?.message || "Resend API error", details: resendData, field: 'form' }), {
			status: 502,
			headers: corsHeaders(origin),
		});
	}

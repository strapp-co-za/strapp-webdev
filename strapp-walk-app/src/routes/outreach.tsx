...
function OutreachPage() {
  ...
  const rendered = tpl ? tpl.body.replace(/\{\{name\}\}/g, client?.name || "there").replace(/\{\{area\}\}/g, client?.area || "") : "";
    ...
    <PageContainer>
      ...
      <Card className="p-5 space-y-3">
        ...
        <div className="flex gap-2">
          <button onClick={() => navigator.clipboard.writeText(rendered)} className="inline-flex items-center gap-2 rounded-md border border-border bg-panel px-3 py-2 text-sm text-foreground hover:bg-panel-elevated">
            <Copy className="h-4 w-4" /> Copy
          </button>
          <a href={`mailto:${client?.email || outreachEmail}?subject=${encodeURIComponent("Quick chat about your visibility")}&body=${encodeURIComponent(rendered)}`} className="inline-flex items-center gap-2 rounded-md bg-foreground px-3 py-2 text-sm font-medium text-background">
            <Mail className="h-4 w-4" /> Email
          </a>
        </div>
        <div className="pt-3 border-t border-border">
          <label className="text-[10px] uppercase tracking-wider text-muted-foreground">Default operator email</label>
          <input className="strapp-input mt-1.5" value={outreachEmail} onChange={(e) => setOutreachEmail(e.target.value)} placeholder="you@strapp.co.za" />
        </div>
      </Card>

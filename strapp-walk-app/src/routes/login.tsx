...
function LoginPage() {
  ...
  function handleLogin(e: React.FormEvent) {
    ...
    fetch("/api/login", {
      ...
      .then(async (res) => {
        ...
        const expiry = stayLoggedIn ? Date.now() + 5 * 24 * 60 * 60 * 1000 : Date.now() + 2 * 60 * 60 * 1000;
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-panel">
      <form onSubmit={handleLogin} className="w-full max-w-xs p-8 bg-card rounded-xl shadow-lg space-y-4">

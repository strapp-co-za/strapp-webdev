...
test.describe('Optimized Core Map Infrastructure Integrity Checks', () => {
  ...
  test('Diagnostic Vector 1: Valid Seeding and Map UI Lifecycle Initialization', async ({ page }) => {
    ...
    await expect(structuralInstructionMarker, 'UI Layout Defect: Map management shell components failed to mount.').toBeVisible({ timeout: 10000 });
  ...
  test('Diagnostic Vector 2: Graceful Fallback Layout Integrity', async ({ page }) => {
    ...
    await page.addInitScript(() => {
      localStorage.setItem('strapp-state', JSON.stringify({ state: { mapsApiKey: "" }, mapsApiKey: "" }));
    });
    ...
    await expect(baseMapPlaceholderText, 'Resilience Failure: The workspace shell crashed or went blank when an empty key was provided.').toBeVisible({ timeout: 5000 });

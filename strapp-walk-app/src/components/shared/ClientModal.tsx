...
export function ClientModal(props: ClientModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center md:items-center bg-black/40" role="dialog" aria-modal="true">
      <form
      ...
      >
        ...
        {currentStepKey === "assign" && (
          <div className="flex flex-col gap-2 mb-2">
            ...
            <div className="flex gap-2">
              <label className={`flex-1 px-3 py-2 rounded-xl border-2 cursor-pointer ${form.campaignType === "day" ? "bg-primary text-background border-primary" : "bg-panel border-border text-foreground hover:border-foreground"}`}>
                <input type="radio" checked={form.campaignType === "day"} onChange={() => setForm(f => ({ ...f, campaignType: "day", campaignId: "" }))} className="mr-2" /> Day-to-day
              </label>
              <label className={`flex-1 px-3 py-2 rounded-xl border-2 cursor-pointer ${form.campaignType === "campaign" ? "bg-primary text-background border-primary" : "bg-panel border-border text-foreground hover:border-foreground"}`}>
                <input type="radio" checked={form.campaignType === "campaign"} onChange={() => setForm(f => ({ ...f, campaignType: "campaign" }))} className="mr-2" /> Campaign
              </label>
            </div>
          ...
          <div className="flex flex-col gap-3">
            <div className="flex flex-col gap-2">
              ...
              <select
                className="strapp-input px-4 py-3 rounded-xl border-2 border-border focus:border-primary"
                value={form.status}
                onChange={e => setForm(f => ({ ...f, status: e.target.value as BusinessStatus }))}
              >
            ...
            {form.status === "active" && (
              <div className="flex flex-col gap-2">
                ...
                <div className="flex gap-2">
                  <label className={`flex-1 px-3 py-2 rounded-xl border-2 cursor-pointer ${form.web ? "bg-primary text-background border-primary" : "bg-panel border-border text-foreground"}`}>
                    <input type="checkbox" checked={form.web} onChange={e => setForm(f => ({ ...f, web: e.target.checked }))} className="mr-2" /> Web Dev
                  </label>
                  <label className={`flex-1 px-3 py-2 rounded-xl border-2 cursor-pointer ${form.google ? "bg-primary text-background border-primary" : "bg-panel border-border text-foreground"}`}>
                    <input type="checkbox" checked={form.google} onChange={e => setForm(f => ({ ...f, google: e.target.checked }))} className="mr-2" /> Google Listing
                  </label>
                </div>
              </div>
            )}
        ...
        {currentStepKey === "google_assets" && (
          <div className="flex flex-col gap-3">
            ...
            <label className="text-xs font-semibold text-muted-foreground">Upload Business Images</label>
            <input
              type="file"
              multiple
              accept="image/*"

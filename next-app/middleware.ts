...
const MARKET_MATRIX = (origin: string, pathname: string, zip: string | null, region: string | null, country: string) : LocaleRoute[] => {
  const list: LocaleRoute[] = [];
  // Local (zip)
  if (zip) list.push({ href: `${origin}${pathname}?loc=${zip}`, hreflang: `${country}-${zip}`, tier: 'local' });
  // Regional (state)
  if (region) list.push({ href: `${origin}${pathname}?region=${encodeURIComponent(region)}`, hreflang: `${country}-${region}`, tier: 'regional' });
...
function pickBestAlternate(alternates: LocaleRoute[], acceptLanguage: string | null, cfCountry: string | null) {
  if (acceptLanguage) {
    try {
  ...
  if (cfCountry) {
    const found = alternates.find(a => a.hreflang.toLowerCase().startsWith(cfCountry.toLowerCase()));
    if (found) return found;
  }
  return alternates[0];
}
...
export function middleware(req: NextRequest) {
  ...
  res.cookies.set({ name: 'x-served-hreflang', value: best.hreflang, maxAge: 60 * 5, httpOnly: true, path: '/' });
